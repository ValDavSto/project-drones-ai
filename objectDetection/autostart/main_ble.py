...
with open("/home/CloudChasers/autostart/logs/autostart_log.txt", "a") as f:
    f.write(f"Main_ble.py Script gestartet um {datetime.datetime.now()}\n")
...
def create_bluetooth_server() -> BluetoothSocket:
    """
    Creates a Bluetooth RFCOMM server socket, binds it to a port, and listens for connections.
    Returns:
        BluetoothSocket: The Bluetooth server socket.
    """
...
# Server setup
server = create_bluetooth_server()
